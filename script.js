    ...
    fetch('ContentTattoo.html')
        ...
        .then(data => {
            ...
            function loadFontsForSelectedCategory(category) {
                ...
                const folderPath = window.location.hostname.includes('github.io') ? '/NeedleG-Web/fonts' : '/NeedleG web/fonts';
                
                // JSON 파일에서 폰트 목록 가져오기
                fetch(`${baseUrl}${window.location.hostname.includes('github.io') ? '/fonts.json' : '/NeedleG web/fonts.json'}`)
                    .then(response => {
            ...
            function loadAndApplyFontToElement(category, fontUrl, fontName, element) {
                ...
                // 폰트 파일의 전체 경로 구성
                const fullFontUrl = `${baseUrl}${window.location.hostname.includes('github.io') ? '/fonts' : '/NeedleG web/fonts'}/${category}/${fontUrl}`;
